...
if(isset($_POST['submit'])){
  if(!empty($_POST['Type'])) {
    ...
    $sql = "UPDATE `utilisateur` SET `IDTypeUtilisateur` = '$IDType' WHERE `utilisateur`.`IdUtilisateur` = $IdUtilisateur";
    $_SESSION['IDTypeUtilisateur'] = $IDType;
...
<form action="ChoixType.php" method="post">
<pre>
<h3>Choisir un type d'utilisateur :</h3>
  <?php  while($ligne=$resultat->fetch(PDO::FETCH_ASSOC)){
      $Nom=$ligne['Fonction'];
      $IDType=$ligne['IDTypeUtilisateur'];
      ?>
     
      <input name= "Type" type="radio" value="<?php echo $IDType ?>"/><label for="<?php $IDType ?>"><?php echo " $Nom"?></label><br />
